...
export class ValidationComponent implements OnInit {
  ...
  constructor(private quranService: QuranService,private audioRecordingService: AudioRecordingService,private sanitizer: DomSanitizer) {
...
//supprime l'audio en cours d'enregistrement
 ...
 private animationFrameId: number | undefined;
    ...
    playAudio(audioUrl: string) {
      if (this.currentlyPlayingAudio && this.currentlyPlayingAudio.src === audioUrl) {
...
removeAudio(index: number) {
  if (index >= 0 && index < this.audioList.length) {
    ...
    if (this.currentlyPlayingAudio && this.currentlyPlayingAudio.src === audioUrlToRemove) {
      this.currentlyPlayingAudio.pause();
      this.currentlyPlayingAudio = null;
    }
...
speedUpAudio(audio: AudioforTransit): void {
  if (audio) {
    ...
    if (audioSrc) {
      ...
      if (this.currentlyPlayingAudio && this.currentlyPlayingAudio.src === audioSrc) {
        ...
        this.currentlyPlayingAudio.playbackRate = this.audioPlaybackRates[audioSrc];
